...
import pytz
from config import host, SECRET_KEY, flask_secret_key
        ...
        if file:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    session['profile_pic_path'] = filename
...
def property():
    ...
    if token and verify_token(token):
        ...
        property_data['profile_pic'] =f"{user_profile_picture}" 
        logged_in = True
        user_data = {
            "first_name": session['first_name'],
            "last_name": session['last_name'],
...
def search():
    ...
    if res.status_code == 200:
        ...
        if token and verify_token(token):
            ...
            profile_picture = f"{session['profile_pic']}"
